  ...
  void onLoading() async {
    ...
    if (activeInternet == "true"
      ...
      if (docs != null &&
        ...
        await helper
            .checkUser(prefs.getString('usuario').toString(),
                prefs.getString('contrasena').toString())
            .then((usuarioExiste) {
          if (usuarioExiste["estado"]=="error") {
            //si no existe el usuario
            responseDialogSaved(Icons.error, Colors.red,
                usuarioExiste["mensaje"], contexto, () async {
    ...
    if (state.currentStep == 0) {
      ...
      if (documentPath == null) {
        ...
        if (activeInternet == "true"
            ...
            : false) {
          var prefs = await SharedPreferences.getInstance();
          dynamic usuarioExiste = await helper.checkUser(
        ...
        if (send) {
          ...
          if (status != null) {
            ...
            if (status == true) {
              //se guarda local
              emitSuccess(successResponse: "Registro guardado con éxito.");
            } else {
              if (status == false) {
